...
# Same Original Crystalball code: Only the png are saved, change x label, add entries.
...
if __name__ == "__main__":
    ...
    ecms = ["160"]
    for ecm in ecms:
        root_file = f"./outputs/stage3_angular_resolution/angular_resolution_ecm{ecm}.root"
        outDir = f"./outputs/resolutions/{time.localtime().tm_year}-{time.localtime().tm_mon}-{time.localtime().tm_mday}-{time.localtime().tm_hour}-{time.localtime().tm_min}/{ecm}"
        os.makedirs(outDir, exist_ok=True)
        currenttime = time.time()

        plots_config = []
        # Making plot configs from the range of jet numbers
        for jetnumber in range(1, 5):
            ...
            xjet = {
                "branch": f"delta_x_j{jetnumber}",  # log(p/m)
                "label": "#sigma_{log(p/m)}",
                "title": "log(p/m)_{gen} - log(p/m)_{reco}",
                "bins": 100,
                "range": (-0.6, 0.6)
            }
            thetajet = {
            ...
            phijet = {
                ...
                "label": "#sigma_{#phi}",
                "title": "#phi_{gen} - #phi_{reco}",
                "bins": 100,
                "range": (-0.02, 0.02)
            }
            filtered_alphajet = {
                "branch": f"filtered_delta_alpha_j{jetnumber}",
                "label": "#sigma_{#alpha}",
                "title": "(E_{jet} - E_{parton})/E_{parton}",
                "bins": 100,
                "range": (-0.25, 0.25)
            ...
            filtered_xjet = {
                "branch": f"filtered_delta_x_j{jetnumber}",  # log(p/m)
                "label": "#sigma_{log(p/m)}",
                "title": "log(p/m)_{gen} - log(p/m)_{reco}",
                "bins": 100,
                "range": (-0.6, 0.6)
            ...
            filtered_thetajet = {
                "branch": f"filtered_delta_theta_j{jetnumber}",
                "label": "#sigma_{#theta}",
                "title": "#theta_{gen} - #theta_{reco}",
                "bins": 100,
                "range": (-0.02, 0.02)
            ...
            filtered_phijet = {
                "branch": f"filtered_delta_phi_j{jetnumber}",
                "label": "#sigma_{#phi}",
                "title": "#phi_{gen} - #phi_{reco}",
                "bins": 100,
                "range": (-0.02, 0.02)
        ...
        for i, config in enumerate(plots_config, 1):
            ...
            json_path = os.path.join(outDir, f"{config['branch']}.json")
            ...
            physicalFitEntries = 0.0
            for entry_bin in range(1, histogram.GetNbinsX() + 1):
                physicalFitEntries += histogram.GetBinContent(entry_bin)

            print(f"  Entries: {histogram.GetEntries()}")
            print(f"  Physical fit entries: {physicalFitEntries:.0f}")
            ...
            fitParameters = {
                ...
                'physical_fit_entries': physicalFitEntries
            }
            ...
            print("FIT RESULTS")
            print("=" * 60)
            print(f"Physical fit entries: {fitParameters['physical_fit_entries']:.0f}")
            ...
            entriesLabel = ROOT.TLatex()
            entriesLabel.SetNDC()
            entriesLabel.SetTextSize(0.040)
            entriesLabel.SetTextFont(42)
            entriesLabel.DrawLatex(0.18, 0.56,
                                f"N_{{fit}} = {fitParameters['physical_fit_entries']:.0f}")
            ...
            text.AddText("")
            text.AddText(f"N_fit = {fitParameters['physical_fit_entries']:.0f}")
            text.AddText("")
        ...
        for branch, res in results.items():
            print(
                f"{branch}: "
                f"N_fit = {res['physical_fit_entries']:.0f}, "
                f"σ = {res['sigma']:.5f} ± {res['sigma_err']:.5f}"
            )
        print("=" * 60)
