...
INPUT_CANDIDATES = [
    os.path.join(SCRIPT_DIR, "outputs", "wmass", "wmass_fit_pvalue_ecm160.root"),
    os.path.join(SCRIPT_DIR, "wmass_fit_pvalue_ecm160.root"),
]
...
OUT_DIR = os.path.join(SCRIPT_DIR, "plots_162p5_raw_4C")
os.makedirs(OUT_DIR, exist_ok=True)

SUMMARY_CSV = os.path.join(OUT_DIR, "summary_162p5_raw_4C.csv")
...
#   prob_4C > 0.03
#
# Both curves are restricted to the visible mass range 40-120 GeV.
...
def plot_mass_overlay(tree, raw_branch, fit4c_branch, outname, xtitle):
    ...
    ymax = max(h_raw.GetMaximum(), h_4c.GetMaximum())
    ...
    leg = ROOT.TLegend(0.15, 0.82, 0.42, 0.93)
    leg.SetBorderSize(0)
    leg.SetFillStyle(0)
    leg.SetTextFont(42)
    ...
    box_4c = draw_stat_box(
        0.78, 0.705, 0.95, 0.85,
        ROOT.kRed + 1, "4C", h_4c
    )
    box_raw = draw_stat_box(
        0.78, 0.545, 0.95, 0.69,
        ROOT.kBlack, "Raw", h_raw
    )

    label = draw_energy_label()

    keepalive = [h_raw, h_4c, leg, box_4c, box_raw, label]
    ...
    print(f"{outname}:")
    print(
        f"  Raw: N={row['raw_entries_40_120']}, "
        f"mean={row['raw_mean_40_120']:.4f}, "
        f"RMS={row['raw_rms_40_120']:.4f}"
    )
    print(
        f"  4C:  N={row['fit4c_entries_40_120']}, "
        f"mean={row['fit4c_mean_40_120']:.4f}, "
        f"RMS={row['fit4c_rms_40_120']:.4f}"
    )
...
def main():
    ...
    rows.append(
        plot_mass_overlay(
            ...
            outname="mass_small_162p5_raw_4C",
    ...
    rows.append(
        plot_mass_overlay(
            ...
            outname="mass_large_162p5_raw_4C",
