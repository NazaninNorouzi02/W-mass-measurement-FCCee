...
// =========================================================
// ISR UPDATE â€” thesis Secs. 7.3.2 and 7.3.2.1
//
...
inline double isrEmax(double sqrts, double mW_ref){
    ...
    // CORRECTION 1: use thesis Eq. 7.41 as written.  The previous
    // implementation used 1 - 4*mW^2/s, which suppresses the allowed ISR
...
// ISR UPDATE â€” photon-augmented kinematic fit.
...
struct AugLagFunctorISR {
    double y0_raw[16];   // raw jet parameters â€” chi2 residual is measured from here
...
// ISR UPDATE â€” numerical inverse of thesis Eq. (7.42).
...
inline ISRFitResult runISRFitCandidate(
    ...
    const int nC = do5C ? 5 : 4;
    // CORRECTION 2: use the same strict numerical convergence requirement
    // for ISR-refit candidates as for the standard fits.
    ...
    // CORRECTION 2: validity means a converged minimizer and tight
    // constraints. The final p-value rejection is left to later analysis.
    result.valid =
...
inline ISRFitResult runBestISRFit(
        ...
        // DEGENERACY DIAGNOSTIC: optional outputs exposing both candidate
        // solutions before the "lower chi2 wins" selection below. Left as
        // nullptr by default so every other call site is unaffected.
        ISRFitResult *outFromZero = nullptr,
        ISRFitResult *outFromMissingPz = nullptr){
    ...
    // DEGENERACY DIAGNOSTIC: hand both raw candidates back to the caller
    // (reconstructWW) before deciding which one wins, so their m1/m2/chi2
    // can be compared event-by-event without changing the selection itself.
    if(outFromZero)      *outFromZero      = fromZero;
    if(outFromMissingPz) *outFromMissingPz = fromMissingPz;
...
reconstructWW(const ROOT::VecOps::RVec<float>& px,
    ...
    // PAIRING UPDATE, corrected version.
    //
    // A previous attempt tried to pick the pairing by running the 4C fit
    // for all three permutations and keeping the lowest chi2_4C. That was
    // wrong: for do5C=false, AugLagFunctor::constraints() and operator()
    // never read a/b/c/d at all (see the struct above -- f[4] is only
    // built "if(do5C)"). The 4C objective and constraints only involve the
    // sum over *all four* jets, so the fit result is identical no matter
    // which permutation label is attached to it. "Lowest chi2 among three
    // near-identical fits" was therefore picking essentially at random
    // (in practice, almost always permutation 0), which is why the mass
    // distributions collapsed (m_large_4C mean ~227 GeV, etc.).
    //
    // Correct, cheap fix: run the 4C fit *once* (pairing-independent), then
    // choose the pairing from Eq. (7.40) using the *fitted* jets instead of
    // the raw ones:
    //   chi2_pairing = (m1_fit - mW_ref)^2 + (m2_fit - mW_ref)^2
    // The fitted jets already have the resolution/ISR-scale corrections
    // applied, so this is a strictly better estimator than the raw masses
    // used previously, with no extra minimization cost (still one fit).

    AugLagFunctor f4C(false);
    f4C.sqrts = sqrts;
    // a/b/c/d are irrelevant to this fit (do5C=false); set to a fixed
    // placeholder so the object is well-defined before pairing is chosen.
    f4C.a = 0; f4C.b = 1; f4C.c = 2; f4C.d = 3;
    ...
    for(int i=0; i<4; i++){
        ...
        f4C.m0[i] = m;
        f4C.y0[4*i+0] = 1.0; f4C.y0[4*i+1] = clampTheta(theta);
        f4C.y0[4*i+2] = wrapPhi(phi); f4C.y0[4*i+3] = std::clamp(x_init, -4.0, 4.0);
    }
    ...
    // CORRECTION 3: require a real constrained stationary point.  The first
    // penalty minimisation may be feasible before the Lagrange multipliers
    // have been updated, so allow at least one multiplier update and require
    // Minuit status==0 before accepting convergence.
    for(int step=0; step<30; step++) {
        ...
        double maxConstraintStep = 0.0;
        for(int k=0; k<4; k++) maxConstraintStep = std::max(maxConstraintStep, std::abs(fvals[k]));
        if(!std::isfinite(maxConstraintStep)) break;
        if(step > 0 && min4C->Status() == 0 &&
           maxConstraintStep < 1.0e-5) break;
        for(int k=0; k<4; k++) f4C.lambda[k] += f4C.mu * fvals[k];
    ...
    for(int k=0; k<4; k++){
        max_constraint_4C = std::max(max_constraint_4C, std::abs(diagnosticConstraints4C[k]));
    }
    ...
    // PAIRING UPDATE: choose (a,b,c,d) from the fitted jets, Eq. (7.40).
    int best_ip = 0;
    double min_chi2_fit = 1e12;
    float m1_fit_cand[3], m2_fit_cand[3];
    for(int ip=0; ip<3; ip++) {
        float m1 = (j_4C[pairings[ip][0]] + j_4C[pairings[ip][1]]).M();
        float m2 = (j_4C[pairings[ip][2]] + j_4C[pairings[ip][3]]).M();
        m1_fit_cand[ip] = m1; m2_fit_cand[ip] = m2;
        double chi2p = (m1-mW_ref)*(m1-mW_ref) + (m2-mW_ref)*(m2-mW_ref);
        if(chi2p < min_chi2_fit) { min_chi2_fit = chi2p; best_ip = ip; }
    }

    int a = pairings[best_ip][0]; int b = pairings[best_ip][1];
    int c = pairings[best_ip][2]; int d = pairings[best_ip][3];
    f4C.a = a; f4C.b = b; f4C.c = c; f4C.d = d;

    float m1_raw = (j_raw[a] + j_raw[b]).M();
    float m2_raw = (j_raw[c] + j_raw[d]).M();
    float m1_4C = m1_fit_cand[best_ip];
    float m2_4C = m2_fit_cand[best_ip];

    // PAIRING DIAGNOSTIC (NEW): gap between best and runner-up post-fit
    // pairing chi2 (Eq. 7.40 on fitted masses). Small gap => ambiguous.
    const int pairing_index = best_ip;
    const int pairing_from_fit = 1; // always chosen from fitted masses now
    double secondBestChi2Pairing = std::numeric_limits<double>::infinity();
    for(int ip=0; ip<3; ip++){
        if(ip == best_ip) continue;
        double chi2p = (m1_fit_cand[ip]-mW_ref)*(m1_fit_cand[ip]-mW_ref)
                     + (m2_fit_cand[ip]-mW_ref)*(m2_fit_cand[ip]-mW_ref);
        if(chi2p < secondBestChi2Pairing) secondBestChi2Pairing = chi2p;
    }
    const double pairing_chi2_gap = secondBestChi2Pairing - min_chi2_fit;

    // Report the true chi2 (without penalty/multiplier terms), i.e.
    // the actual Least-Squares estimator from Eq. (7.5) of the thesis.
    // Pairing-independent, as explained above.
    double dpar4C[16];
    ...
    // CORRECTION 3: an unconverged standard fit has no physical p-value.
    const bool standard_4C_valid =
        status_4C == 0 &&
        std::isfinite(chi2_4C) &&
        max_constraint_4C < 1.0e-5;
    ...
    // CORRECTION 4: same strict convergence logic for the standard 5C fit.
    for(int step=0; step<30; step++) {
        min5C->Minimize();
    ...
    // CORRECTION 4: an unconverged standard fit has no physical p-value.
    ...
    // DEGENERACY DIAGNOSTIC: captures the two 5C+ISR candidate solutions
    // (from y=0 and from the missing-pz-derived start) separately, so we
    // can test whether they converge to similar chi2 but different masses.
    ISRFitResult diag5C_fromZero;
    ISRFitResult diag5C_fromMissingPz;
    ...
    if(standard_5C_valid && prob_5C < 0.03 &&
       EMAX_ISR > 1.0e-9){
        const ISRFitResult best5CISR = runBestISRFit(
            ...
            "5C_zero",
            "5C_pz",
            &diag5C_fromZero,        // DEGENERACY DIAGNOSTIC
            &diag5C_fromMissingPz    // DEGENERACY DIAGNOSTIC
        );
    ...
    return ROOT::VecOps::RVec<float>{
        ...
        // DEGENERACY DIAGNOSTIC (NEW): the two 5C+ISR candidate solutions,
        // reported separately and *before* the "lower chi2 wins" choice
        // made inside runBestISRFit. Only meaningful when isr_5C_attempted
        // == 1; otherwise these are the ISRFitResult default sentinels
        // (valid=0, chi2=+inf, m1=m2=0).
        (float)diag5C_fromZero.m1,          // 108
        (float)diag5C_fromZero.m2,          // 109
        (float)diag5C_fromZero.chi2,        // 110
        (float)diag5C_fromZero.valid,       // 111
        (float)diag5C_fromMissingPz.m1,     // 112
        (float)diag5C_fromMissingPz.m2,     // 113
        (float)diag5C_fromMissingPz.chi2,   // 114
        (float)diag5C_fromMissingPz.valid,  // 115

        // PAIRING DIAGNOSTIC (NEW)
        (float)pairing_index,       // 116: which permutation won (0,1,2)
        (float)pairing_from_fit,    // 117: 1 if chosen by post-4C-fit chi2, 0 if fallback to raw-mass Eq.(7.40)
        (float)pairing_chi2_gap     // 118: chi2_4C(2nd best) - chi2_4C(winner); small/negative-sentinel(-1) => ambiguous or no runner-up converged
    };
...
def build_covariance_matrix(json_path=_JSON_PATH):
    ...
    # â€” same as the parametrization in Eq. (7.23) of the thesis.
    jet_vars = [
        ("alpha", 0),
        ("theta", 1),
        ("phi",   2),
...
# CORRECTION 5: do not add an absolute 1e-8 diagonal term.  The JSON sigmas
# define the covariance scale; adding a fixed variance changes the smallest
# angular resolutions and biases the chi2/pull scale.
...
class RDFanalysis:
    ...
    def analysers(df):
        df = (
            ...
            .Define("delta_m_4C_pre5C", "m_large_4C - m_small_4C")
            ...
            # DEGENERACY DIAGNOSTIC: the two 5C+ISR candidates (y=0 start
            # and missing-pz-derived start) reported separately, before the
            # "lower chi2 wins" choice inside runBestISRFit. Compare
            # m1_5C_isr_zero/m2_5C_isr_zero vs m1_5C_isr_pz/m2_5C_isr_pz for
            # events where both *_valid flags are 1 and chi2 values are
            # close, to test whether the 5C+ISR fit has near-degenerate
            # solutions with different reconstructed masses.
            .Define("m1_5C_isr_zero",   "all_results[108]")
            .Define("m2_5C_isr_zero",   "all_results[109]")
            .Define("chi2_5C_isr_zero", "all_results[110]")
            .Define("valid_5C_isr_zero","all_results[111]")
            .Define("m1_5C_isr_pz",     "all_results[112]")
            .Define("m2_5C_isr_pz",     "all_results[113]")
            .Define("chi2_5C_isr_pz",   "all_results[114]")
            .Define("valid_5C_isr_pz",  "all_results[115]")

            # PAIRING DIAGNOSTIC (NEW): pairing_index/pairing_from_fit/
            # pairing_chi2_gap describe how the (a,b,c,d) permutation was
            # chosen (post-4C-fit chi2 vs. raw-mass fallback) and how close
            # the decision was.
            .Define("pairing_index",     "all_results[116]")
            .Define("pairing_from_fit",  "all_results[117]")
            .Define("pairing_chi2_gap",  "all_results[118]")
    ...
    def output():
        branches = [
            ...
            "m_small_4C", "m_large_4C", "delta_m_4C_pre5C",
            "chi2_4C", "prob_4C",
            ...
            # DEGENERACY DIAGNOSTIC (NEW): both 5C+ISR candidate solutions,
            # before the "lower chi2 wins" selection.
            "m1_5C_isr_zero", "m2_5C_isr_zero",
            "chi2_5C_isr_zero", "valid_5C_isr_zero",
            "m1_5C_isr_pz", "m2_5C_isr_pz",
            "chi2_5C_isr_pz", "valid_5C_isr_pz",

            # PAIRING DIAGNOSTIC (NEW)
            "pairing_index", "pairing_from_fit", "pairing_chi2_gap",
        ]
